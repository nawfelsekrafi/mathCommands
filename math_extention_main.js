...
window.onload = (event) => {
  ...
  // Get the saved latex string from the chrome storage
  chrome.storage.sync.get("save_latex", ({ save_latex }) => {
    //console.log ("in window load - save_queue: " + save_queue +" type " + typeof save_queue);

    if(save_latex.length == 0){
        //console.log("in window load - created new queue " + save_latex );
        chrome.storage.sync.set({ "save_latex" : ""});
    }else{
        //console.log("in window load - deserialize  queue " + save_latex );
      loadLatexToOutputArea(save_latex);
    }
    //console.log("loaded the latex: " + mathFieldArray[mathFieldFocus].latex());
  });

  chrome.storage.sync.get("save_history", ({ save_history }) =>{
    if(save_history.length == 0){
      mathFieldArray[0].latex('x=\\frac{-b\\pm\\sqrt{b^2-4ac}}{2a}');
      saveToHistory();
      clear();
    }else{
      try{
        latexHistory = JSON.parse(save_history.toString());
      }catch{
        mathFieldArray[0].latex('x=\\frac{-b\\pm\\sqrt{b^2-4ac}}{2a}');
        saveToHistory();
        clear();
  ...
  // Get the font size form the settings - this is the size of the fonts in the DXImageTransform
  // that is copied
  chrome.storage.sync.get(['StroedFontSize'], function(result) {
    console.log('StroedFontSize currently is ' + result.StroedFontSize);
    font_size = result.StroedFontSize;
  });

  chrome.storage.sync.get({'StoredFontColor' : BLACK_COLOR}, function(result) {
    console.log('StoredFontColor currently is ' + result.StoredFontColor);
    font_color = result.StoredFontColor;
  });

  //check if this is the first time and the user didn't click dont_show_again
  chrome.storage.sync.get({dont_show_welcome: false}, function(result) {
    if(result.dont_show_welcome == false){
      let welcome_dialog = document.getElementById("welcome_dialog");
      welcome_dialog.showModal();
    }
  });

  // Count how many time this user openned the extention and ask for review on the 20 and 150 time
  chrome.storage.sync.get({UseCount : 1}, function(result) {
  //alert('UseCount currently is ' + result.UseCount);

    // Open a dialog for review
    let review_dialog = document.getElementById("review_dialog");
    //review_dialog.showModal();

    // First time the extention is used
    if (result.UseCount == 1){
        trackEvent('first use', 'first_use_of_extention');
    }

    // Open a dialog for tutorial/video
    count = result.UseCount + 1;
    chrome.storage.sync.set({UseCount: count});

    // This is for debug to see the review dialog - we need to remove this code later

    if(count == REVIEW_FIRST_TIME){
      trackEvent('Review dialog', 'open_review_dialgo');
      review_dialog.showModal();
    }
    if(count == REVIEW_SECOND_TIME){
      trackEvent('Review dialog', 'open_review_dialgo');
      review_dialog.showModal();
    }
